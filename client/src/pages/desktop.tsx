...
/**
 * IMPORTANT:
 * - This file is a drop-in replacement for: client/src/pages/desktop.tsx
 * - It includes fixes:
 *   1) Taskbar styled like a centered dock (Windows 11-ish), slightly raised, rounded, ~70% glass
 *   2) Resizer bug fix (was referencing w.x / w.y incorrectly)
 *   3) Installed apps persist + appear on Desktop after install (localStorage)
 *   4) Smoother window motion (Framer "layout" enabled)
 */
...
const LS_KEYS = {
  ...
  installedApps: "webos.installedApps.v1",
} as const;

/** Add the store apps here too, so TS is happy */
type AppId =
  | "notes"
  | "files"
  | "browser"
...
function AppStore({
  ...
  useEffect(() => {
    // sync install state
    setApps((prev) =>
      prev.map((a) => ({ ...a, isInstalled: installedApps.includes(a.id) }))
    );
  }, [installedApps]);
...
function StartMenu({
  ...
  return (
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-[8000]" onMouseDown={onClose} data-testid="overlay-start-menu">
          ...
          <motion.div
            ...
            className={cn(
              "absolute left-1/2 -translate-x-1/2 bottom-[92px] w-[520px] rounded-3xl glass p-4 flex flex-col gap-4",
              isMobile && "left-3 right-3 w-auto bottom-[92px] translate-x-0",
            )}
          ...
          >
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-white/10 border-none rounded-2xl pl-10 py-2.5 text-sm focus:ring-2 ring-[var(--desktop-accent)] outline-none transition"
              />
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-1">
                {([
                  ...
                  .filter((id) => appMeta(id).title.toLowerCase().includes(search.toLowerCase()))
                  .map((id) => {
                    ...
                    return (
                      <button
                        key={id}
                        className="group flex flex-col items-center gap-1.5 rounded-2xl p-3 hover:bg-white/10 transition"
                        onClick={() => {
                          onOpenApp(id);
                          onClose();
                        }}
            ...
            <div className="mt-auto border-t border-white/10 pt-4 flex items-center justify-between px-1">
              <div className="flex items-center gap-3">
                <div className="h-8 w-8 rounded-full bg-[var(--desktop-accent)] flex items-center justify-center text-[10px] font-bold text-white shadow-inner">
                  JD
                </div>
                <div className="text-[11px] font-semibold">User Account</div>
              </div>
              <button className="p-2 rounded-2xl hover:bg-white/10 transition"><Cog className="h-4 w-4" /></button>
            </div>
          </motion.div>
        </div>
      )}
...
function FilesApp() {
  const [currentFolder, setCurrentFolder] = useState("This PC");
            ...
            <div className="rounded-2xl border glass p-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{wifiConnected ? "Connected" : "Disconnected"}</div>
                </div>
              </div>
    ...
    <div className="flex items-center gap-1 pr-2">
      <Popover>
        <PopoverTrigger asChild>
          <button className="flex items-center gap-1.5 rounded-2xl px-2 py-1.5 hover:bg-white/10 transition">
        ...
        <PopoverContent className="w-80 glass p-4 mb-2 rounded-3xl" side="top" align="end">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button
                  ...
                  className={cn(
                    "flex flex-col items-center gap-2 p-3 rounded-2xl transition",
                ...
                <button
                  className="flex flex-col items-center gap-2 p-3 rounded-2xl bg-white/10 hover:bg-white/15 transition"
                >
      ...
      <Popover>
        <PopoverTrigger asChild>
          <button className="flex flex-col items-end px-2 py-1 rounded-2xl hover:bg-white/10 transition">
        ...
        <PopoverContent className="w-[320px] glass p-4 mb-2 rounded-3xl" side="top" align="end">
          <div className="space-y-4">
            <div className="flex items-center justify-between px-1">
              ...
              {Array.from({ length: daysInMonth(currentMonth.getFullYear(), currentMonth.getMonth()) }).map((_, i) => {
                ...
                return (
                  <button
                    key={i}
                    className={cn(
                      "h-9 w-full rounded-2xl text-[12px] transition flex items-center justify-center",
...
export default function Desktop() {
  ...
  const [installedApps, setInstalledApps] = useLocalStorageState<AppId[]>(LS_KEYS.installedApps, []);
  ...
  useEffect(() => {
    ...
    document.documentElement.style.setProperty("--desktop-accent", `hsl(${settings.accent})`);
    // ~70% transparency feel for the taskbar windows
    document.documentElement.style.setProperty("--glass-opacity", settings.transparency ? "0.70" : "0.95");
  ...
  // Fix resize math bug + smoother drag/resize
  useEffect(() => {
    function onPointerMove(e: PointerEvent) {
      if (draggingId.current) {
        ...
        setWindows((prev) =>
          prev.map((win) => {
            ...
            return { ...win, x: clamp(nx, 10, maxX), y: clamp(ny, 10, maxY) };
          }),
        );
      }
      ...
      if (resizingId.current) {
        ...
        setWindows((prev) =>
          prev.map((win) => {
            if (win.id !== id) return win;
            if (win.maximized || isMobile) return win;
            const dx = e.clientX - resizeStart.current.x;
            const dy = e.clientY - resizeStart.current.y;

            const maxW = window.innerWidth - win.x - 12;
            const maxH = window.innerHeight - win.y - 110;

            const nw = clamp(resizeStart.current.w + dx, 320, maxW);
            const nh = clamp(resizeStart.current.h + dy, 220, maxH);
  ...
  const desktopIcons = useMemo(() => {
    const base: Array<{ id: AppId; label: string; Icon: any }> = [
      { id: "notes", label: "Notes", Icon: FileText },
      { id: "files", label: "Files", Icon: Folder },
      { id: "browser", label: "Browser", Icon: Globe },
      { id: "settings", label: "Settings", Icon: Cog },
      { id: "store", label: "App Store", Icon: Download },
    ];

    const extra = installedApps
      .filter((id) => !base.some((b) => b.id === id))
      .map((id) => {
        const meta = appMeta(id);
        return { id, label: meta.title, Icon: meta.icon };
      });
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 flex h-full flex-col">
                ...
                .map((w) => {
                  ...
                  return (
                    <motion.div
                      key={w.id}
                      layout
                      initial={settings.animations ? { opacity: 0, scale: 0.94, y: 18 } : {}}
                      animate={settings.animations ? { opacity: 1, scale: 1, y: 0 } : {}}
                      exit={settings.animations ? { opacity: 0, scale: 0.94, y: 18 } : {}}
                      transition={{ type: "spring", damping: 26, stiffness: 360 }}
                      className={cn(
                        "absolute rounded-[18px] window-shadow overflow-hidden will-change-transform",
          ...
          {/* Centered dock taskbar */}
          <div
            className={cn(
              "fixed left-1/2 -translate-x-1/2 z-[9000]",
              "bottom-4",
              "h-12",
              "w-[min(760px,calc(100%-24px))]",
              "rounded-3xl",
              "px-3 flex items-center justify-between",
              "border border-white/20",
              "shadow-[0_18px_60px_rgba(0,0,0,0.45)]",
              settings.transparency ? "bg-white/70 dark:bg-black/45 backdrop-blur-xl" : "bg-card",
            )}
          >
            <div className="flex items-center gap-2">
              {/* Start button - cleaner */}
              <button
                onClick={() => setStartOpen(!startOpen)}
                className={cn(
                  "group relative grid h-10 w-10 place-items-center rounded-2xl transition-all",
              ...
              >
                <div className="grid grid-cols-2 gap-0.5 p-1.5">
                  <div className="h-2.5 w-2.5 rounded-sm bg-[var(--desktop-accent)]" />
                  <div className="h-2.5 w-2.5 rounded-sm bg-[var(--desktop-accent)] opacity-80" />
                  <div className="h-2.5 w-2.5 rounded-sm bg-[var(--desktop-accent)] opacity-65" />
                  <div className="h-2.5 w-2.5 rounded-sm bg-[var(--desktop-accent)] opacity-45" />
                </div>
              </button>
                ...
                {windows.map((w) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "relative h-10 w-10 rounded-2xl transition flex items-center justify-center",
                        active ? "bg-white/15 shadow-sm" : "hover:bg-white/8"
                      )}
                      aria-label={meta.title}
                    >
                      {Icon && <Icon className="h-5 w-5" />}
                      {active && <div className="absolute bottom-1 left-1/2 -translate-x-1/2 h-0.5 w-2 bg-[var(--desktop-accent)] rounded-full" />}
                    </button>
                  );
                })}
              </div>
